...
- **SmolVLM/Idefics3 vision SFT now reaches real training batches (#302 by
  @Amix29).** Soup keeps LLaVA messages and PIL images together until collation,
  converts legacy `<image>` markers to structured multimodal content, and lets
  the processor produce image-token expansion plus architecture-specific pixel
  tensors. The vision path uses the Transformers trainer with this collator so
