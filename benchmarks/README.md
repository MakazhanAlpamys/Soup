...
| [`stream_probe.py`](harness/stream_probe.py) | [`probe-rtx5070-what-bounds-streaming.md`](probe-rtx5070-what-bounds-streaming.md) | What bounds the streamed step on THIS card? Builds the model through the shipped path, replicates the two shipped copy paths with CUDA events added (a source guard refuses to run if the shipped body drifts), and runs the ceiling / step / sweep / interleaved-ablation modes of the v0.73.0 probe, writing JSON after every point. `--tier disk` measures the shipped DiskSource; `--lazy-shard-handles` is the documented sharding workaround for the Windows mapping limit | CUDA GPU; model weights; a 7B NF4 run is ~10 min all modes; the 70B-shaped cold-disk run is ~1 h |
