...
**Honest scope:**
- **RAM tier + disk overflow (v0.72.3).** `stream_source: auto` picks RAM when it fits, falls back to NVMe disk when not; SATA/HDD rejected. "Fits" has two ceilings (#622): the store plus resident extras must fit under 70% of *available* RAM **and** under 55% of *total* physical RAM — a page-locked store is unevictable-class memory the kernel cannot swap or reclaim, so free RAM alone green-lit a 16.10 GB store + 3.11 GB resident on a 30 GB host with 27 GB "available" and the run was OOM-killed (exit 137) before the first step. When the absolute ceiling is what refuses, the fallback note names it; a forced `stream_source: ram` over that ceiling is refused before sharding, with the ceiling in the message. Correctness verified; disk performance unmeasured on the reference box. A paravirtual (virtio) disk reports `rotational=1` with no media hint, so a genuinely NVMe-backed cloud disk was misread as an HDD and refused (#365); detection now measures a bounded O_DIRECT sequential read when the rotational flag is unreliable and admits NVMe-class throughput (>= 1 GB/s), while a genuinely slow disk stays rejected. Set `training.stream_disk_kind: nvme` (or `ssd`/`hdd`) to override when detection is still wrong — the resolved value is printed beside what was detected.
...
- **Disk tier chosen (or `stream_source: ram` refused) while plenty of RAM looks free** — the #622 absolute ceiling: the store plus resident extras must also fit under 55% of *total* physical RAM, because page-locked memory cannot be swapped or reclaimed and the kernel OOM-kills instead of refusing. Freeing RAM does not move this ceiling; `stream_source: auto` (the disk tier) or a smaller base does.
