...
|---|---|---|
| `moe_lora` | `sft`, `pretrain`, `tts`, (since #798) `dpo`, `kto`, `orpo`, `simpo`, `grpo`, (since #1099) `ipo`, `bco`, `reward_model`, `ppo`, `embedding`, `online_dpo`, and (since #1151) `classifier`, `reranker`, `cross_encoder`, `distill`, `unlearn` | refused at config load on `asr` and `moe_lora_routing`, naming the task; still loads unread on `prm` |
...
**`moe_lora` on the remaining LoRA tasks (#1099).** #798 left it loading but unread on `ipo`, `bco`, `reward_model`, `ppo` and `embedding`, and `online_dpo` had the same gap. All six build their adapter through the same `build_lora_config` path, so they were wired to the same helper rather than refused. On `embedding` it applies only with `lora.r >= 1`; at `r: 0` that trainer full-fine-tunes and builds no adapter for the flag to select. #1151 closed the remainder: `classifier`, `reranker` and `cross_encoder` (one trainer), `distill` and `unlearn` build their adapter the same way and are wired to the same helper; `asr`, which trains only Whisper (no experts), and `moe_lora_routing`, which builds no LoRA adapter, refuse the flag at config load. `prm` builds no adapter either and still accepts the flag unread, pending a ruling on #1151. `preference` is covered through the trainers it dispatches to, and `tts` through the SFT trainer it subclasses.
