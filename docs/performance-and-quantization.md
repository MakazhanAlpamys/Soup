...
- **Llama / Qwen / Qwen3.5 MoE text / Mistral / Gemma / Gemma2 / Gemma3-Text / Phi / Phi3** (`qwen3_5_moe` and `qwen3_5_moe_text` route through the qwen3 streamer), `task: sft`, `backend: transformers`, `modality: text`. The original list is verified bit-exact in bf16 and NF4; the Qwen3.5 MoE path has live streamed-training validation on `Qwen/Qwen3.5-35B-A3B`, but no resident bit-exact control because the author did not have hardware that could load it resident.
