...
gradient, the loss is the mean over the ranked pairs of each batch, and the share of unranked
pairs is logged as `judge/invalid_rate`, with a WARNING the first time it happens. If the judge
ranks none of 32 pairs in a row (for example because its server is down), training stops with
an error that names the judge, and a shorter run in which it ranked no pair at all fails the
same way instead of saving an adapter. Before this, TRL trained every such pair as if the
second completion had won (#1225).
